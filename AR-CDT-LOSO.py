...
class MSARConv(nn.Module):
    # 核心改动：接收一个kernel_sizes列表，而不是单一的kernel_size
    def __init__(self, inc, outc, kernel_sizes=[3, 5, 7], padding_modes='zeros'):
        super(MSARConv, self).__init__()
        # 尺度的数量由kernel_sizes列表的长度决定
        self.num_scales = len(kernel_sizes)
        self.inc = inc
        self.outc = outc

        # p_conv现在需要为所有尺度生成偏移量
        ...
        # --- 核心创新点 ---
        # 我们根据kernel_sizes列表，创建不同感受野大小的卷积层
...
class ARConvNet(nn.Module):
    ...
    def __init__(self, num_features, num_classes, bottleneck_dim, arconv_out_c, dropout_rate,
        ...
        if ablate_arconv:
            ...
            print("--- 核心模块已被替换为 SimpleCNN (消融AR-Conv) ---")
        else:
            # 否则，正常创建MSARConv，并使用可变的视角数量
            self.spatial_expert = MSARConv(            inc=1, 
            outc=arconv_out_c,
            kernel_sizes=[3, 5, 7])
            print(f"--- 正在使用 MSARConv (视角数量: {arconv_views}) ---")
        ...
        if ablate_cd_tanh:
            ...
            print("--- 分类器激活模块已被替换为 BatchNorm1d + GELU (消融CD-Tanh) ---")
        else:
            # 否则，使用我们最终的创新模块
            self.final_classifier = nn.Sequential(
                nn.Linear(final_dim, classifier_hidden_dim),
...
def run_arconvnet_fold(X_train_raw, y_train_raw, X_val_raw, y_val_raw, num_classes, args):
    """为ARConvNet模型运行单次交叉验证折数。(最终版: 修正了推理速度测量)"""
    ...
    X_val = clr_transform(X_val_raw)
    
    # 2. 转换为Tensor
    X_train_tensor = torch.tensor(X_train, dtype=torch.float).to(device)
    y_train_tensor = torch.tensor(y_train_raw, dtype=torch.long).to(device)
    X_val_tensor = torch.tensor(X_val, dtype=torch.float).to(device) # <--- 用于批量推理
    ...
    model = ARConvNet(
        ...
        ablate_cd_tanh=args.ablate_cd_tanh,
        ablate_arconv=args.ablate_arconv,
        arconv_views=args.arconv_views
    ).to(device)
    
    # --- 指标: 1. 模型大小 ---
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"  [Metrics] AR-CDT - 可训练参数: {total_params}")
    
    criterion = FocalLoss(alpha=torch.ones(num_classes).to(device), gamma=args.gamma) # 临时的alpha，仅用于evaluate
    optimizer = optim.Adam(model.parameters(), lr=args.learning_rate, weight_decay=args.weight_decay)

    # 4. 训练循环
    best_val_auc = 0.0
    epochs_no_improve = 0
    best_y_true, best_y_prob = None, None

    # --- 指标: 2. 训练时间 ---
    print(f"  [Metrics] AR-CDT - 开始训练 {args.epochs} 轮...")
    ...
    for epoch in range(args.epochs):
        ...
        # 重新计算真实的alpha
        num_samples_per_class = torch.bincount(y_train_tensor, minlength=num_classes).float()
        alpha_weights = 1.0 / (num_samples_per_class + 1e-9)
        alpha_weights = alpha_weights / alpha_weights.sum()
        criterion.alpha = alpha_weights.to(device) # 更新FocalLoss的alpha
        
        loss = criterion(outputs, y_train_tensor)
        loss.backward()
        optimizer.step()
        
        # 这里的 evaluate 返回 (loss, probs, y_true_np, y_prob_np)
        _, _, val_true, val_probs = evaluate(model, X_val_tensor, y_val_tensor, criterion, device)
        
        current_auc = roc_auc_score(val_true, val_probs[:, 1] if num_classes == 2 else val_probs, multi_class='ovr')

        if current_auc > best_val_auc:
            best_val_auc = current_auc
            best_y_true = val_true
            best_y_prob = val_probs
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
        
        if epochs_no_improve >= args.patience:
            print(f"  [Metrics] AR-CDT - 在第 {epoch+1} 轮早停。")
            break
            
    end_train_time = time.perf_counter()
    training_duration = end_train_time - start_train_time
    print(f"  [Metrics] AR-CDT - 训练耗时: {training_duration:.2f} 秒")
    ...
    if device.type == 'cuda':
        ...
        print(f"  [Metrics] AR-CDT - 峰值GPU VRAM: {peak_memory_mb:.2f} MB")
        
    # --- 指标: 4. 推理速度 (批量) ---
    print(f"  [Metrics] AR-CDT - 正在测量 {len(X_val_tensor)} 个样本的批量推理速度...")
    model.eval() 
    with torch.no_grad():
        # "热身" (Warm-up) - 确保CUDA已初始化，不计入时间
        _ = model(X_val_tensor) 
        
        # 真正的测量
        start_infer_time = time.perf_counter()
        _ = model(X_val_tensor) # <--- 预测整个验证集批次
        end_infer_time = time.perf_counter()

    total_batch_time = end_infer_time - start_infer_time
    num_samples = len(X_val_tensor)
    avg_inference_time_s = total_batch_time / num_samples # 平均每样本的【秒】数
    avg_inference_time_ms = avg_inference_time_s * 1000   # 转换为【毫秒】
    
    print(f"  [Metrics] AR-CDT - 批量总耗时: {total_batch_time:.4f}s / {num_samples} 样本")
    print(f"  [Metrics] AR-CDT - 平均推理速度: {avg_inference_time_ms:.8f} 毫秒/样本")
    
    # 返回所有指标
    return best_val_auc, best_y_true, best_y_prob, total_params, training_duration, avg_inference_time_ms, peak_memory_mb

def main():
    parser = argparse.ArgumentParser(description="Final Ablation Study Model")
    ...
    parser.add_argument('--n_repeats', type=int, default=10, help="交叉验证的重复次数")
    ...
    parser.add_argument('--ablate_arconv', action='store_true',
                        help="如果设置此项, 将禁用 MultiViewARConv 模块, 换回一个简单的CNN。")
    parser.add_argument('--arconv_views', type=int, default=3, choices=[1, 2, 3],
                        help="设置 MultiViewARConv 中的视角数量 (默认为3)。")
    ...
    groups = df_metadata.set_index('sampleID').loc[common_samples]['studyID'].values  
    ...
    print("--- 3. 开始执行 Leave-Study-Out (LSO) 交叉验证 ---")

    logo = LeaveOneGroupOut()
    fold_aucs, fold_accs, fold_f1s, fold_recalls = [], [], [], []

    for fold_idx, (train_idx, val_idx) in enumerate(logo.split(X_processed, y_encoded, groups=groups)):
        unique_group = np.unique(groups[val_idx])[0]
        print(f"\n--- 第 {fold_idx+1}/{logo.get_n_splits(groups=groups)} 折 (留出研究: {unique_group}) ---")

        # 如果该研究验证集只有一个类别，跳过
        if len(np.unique(y_encoded[val_idx])) < 2:
            print(f"  ⚠️  跳过研究 {unique_group}，因为验证集中只有一个类别。")
            continue
